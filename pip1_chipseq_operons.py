...
def print_full(x): # print binary-based sequences
    y = pd.DataFrame(x)
    y.index = [biocoder.itoseq(x) for x in y.index]
    y["revcomp"] = [biocoder.revcompstr(x) for x in y.index]
    return y
...
def read_chip(pbm_format,norm_method,kmer=6): # reading PBM input and apply transformation to scores and binary seq
    ...
    seqbin = [biocoder.seqtoi(x) for x in pbm_format['sequence']] #  PBM içindeki her bir sekansı binary gösterimine çevirir
    oligfreq = biocoder.nonr_olig_freq(seqbin,kmer,nonrev_list) # feature vs sekans içeren count table oluşturur
...
def mypredict(seq1, seq2, params,cov_matrix):
    ref = biocoder.nonr_olig_freq([biocoder.seqtoi(seq1)],6,nonrev_list)  # from N
    mut = biocoder.nonr_olig_freq([biocoder.seqtoi(seq2)],6,nonrev_list)  # to N
