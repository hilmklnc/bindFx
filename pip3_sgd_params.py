...
def read_chip(pbm_format,norm_method,kmer=6): # reading PBM input and apply transformation to scores and binary seq
    ...
    oligfreq = bio.nonr_olig_freq(seqbin) # feature vs sekans içeren count table oluşturur
...
def apply_sgd(df):
    ...
    # print_motif = print_full(print_motif["Weights"].sort_values(ascending=False))
    #sgd,params,print_motif,cov
    return params,cov # return[1] = coefficients return[-1] = covariance
def save_params(train_file):
    ...
    with open(f"outputs/params_new/p_{train_file.split(".txt")[0]}.pkl", "wb") as file:
...
def mypredict(seq1, seq2, k, params,cov_matrix):
    ref = bio.nonr_olig_freq([bio.seqtoi(seq1)])  # from N
    mut = bio.nonr_olig_freq([bio.seqtoi(seq2)])  # to N
...
trainsets = os.listdir(file_path)[350:]
...
if __name__ == "__main__":
    time_elapsed = run_save(8)
    print("\n")
    print(time_elapsed/60,"min")
    print("\n ---ENDED---")


# with open("outputs/params403/p_ChIPseq_ENCFF156UNQ_HepG2_cell line_HMG20A.pkl", "rb") as file:
#     file_new = pickle.load(file)
#
#
# with open("outputs/models403/p_ChIPseq_ENCFF156UNQ_HepG2_cell line_HMG20A.pkl", "rb") as file2:
#     file_old = pickle.load(file2)
#
