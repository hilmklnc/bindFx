...
# x-axis -- p values y-axis fold-change
# 10 TFs model en iyi ChIP-seq bed file kullan
# Sana'df : TF-gain, TF-loss, TF-gain detail, T-loss detail)
...
def single_vcf_to_bed(vcf_file): # convert vcf file to bed format containing length of 60 bps
    ...
    bed_format = pd.concat([vcf_data[0], (vcf_data[1] - 30), (vcf_data[1] + 30)], axis=1)  # getting -30+ sequences
    bed_format.columns = ["chr", "start", "end"]
    bed_format.to_csv(f"{sample_name}_bed.txt", header=False, index=False, sep="\t")
...
def single_vcf_seq(vcf_file,fasta_file): # adding wild-type and altered 60 bps sequences into VCF
    ...
    vcf_data.to_csv(f"{sample_name}_seq.csv", index=False)
...
def glob_bed_to_fasta(bed_files,genome):
    ...
    for bed in beds:
        vcf_name = bed.split("/")[2].split("_")[0]
        with open(bed, "r") as bed_file, open(f"{vcf_name}_fasta.txt", "w") as output_file:
